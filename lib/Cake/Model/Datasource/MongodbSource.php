...
App::uses('SqlCompatible', 'Model/Behavior');
...
*/
        ...
        if (!empty($Model->mongoSchema) && is_array($Model->mongoSchema)) {
            ...
            $Model->Behaviors->attach('Schemaless');
            if (!$Model->data) {
 ...
 */
    ...
    public function customGroup($params, $Model = null) {
        $tableName = $Model;
        ...
        if ($this->fullDebug) {
            $this->logQuery("db.{$tableName}.group( :key, :initial, :reduce )", $params);
        }
 ...
 */
    public function updateAll(&$Model, $fields = null,  $conditions = null) {
        ...
        foreach( $conditions as $key => $value ) {
            if( $key == '_id' ) $conditions[$key] = $this->_convertId( $value, 1 );
        }
        $this->_prepareLogQuery($Model); // just sets a timer
        try{
            if ($this->_driverVersion >= '1.3.0') {
 ...
 */
    public function read(Model $Model, $query = array(), $recursive = null) {
        ...
        if( count($conditions) == 1 && array_key_exists('aggregate', $conditions)) {
            ...
            $limit = $limit > 0 ? $limit: 9999999999;
        }
     ...
     */
        public function query() {
            ...
            if( isset( $query['custom'] ) ) {
                if( strtolower( $query['custom'] ) == 'group' ) {
                    $return = $this->customGroup( $args[0][0], $args[0]['model'] );
                    return $return;
                }
            }
            $params = array();
